...
class WOUtility:
   ...
   #############################fetch overdued workorders ######################################
    ...
    def seachWoByTags(searchStr):
        ...
        return WorkOrder.objects.filter(summaryofIssue__isnull=False,isScheduling=False,visibile=True).filter(Q(summaryofIssue__contains=searchStr)|Q(woAsset__assetName__contains=searchStr)).order_by('-id')
...
#######################Simple Report######################
    ...
    def getOpenWorkOrdersDetailReport(start,end,assignedUser,asset,assetCategory,maintenanceType,priority):
        ...
        if(len(assignedUser)>0):
            wo=WorkOrder.objects.filter(assignedToUser__id__in=assignedUser,visibile=True,datecreated__range=(start,end),isScheduling=False).exclude(woStatus__in=(7,8))
        else:
